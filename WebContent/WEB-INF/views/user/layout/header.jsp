...
$(function(){
	...
	$('#btnReg').click(function(){
		location.href = '/user/join/memberRegist.do';
	});
});

function loginCheck(){
	if(eval('${!empty LOGIN_MEMBERINFO}')) {
		$('#memberinfo-loginForm').hide();
		$('#memberinfo-logoutForm').show();
		$('#member-name').text('${LOGIN_MEMBERINFO.mem_name}');
	} else {
		$('#memberinfo-loginForm').show();
		$('#memberinfo-logoutForm').hide();
	}
}
</script>
</head>
...
<div id="header-topbar-option" class="page-header-topbar">
	<nav id="topbar" role="navigation" style="margin-bottom: 0;" data-step="3" 
		...
		<div class="navbar-header">
			...
			<a id="logo" href="/user/main.do" class="navbar-brand">
				<span class="logo-text text-yellow" style="font-size: 16px;">(재단법인)대덕인재개발원</span>
			</a>
		</div>
		<div class="topbar-main" id="header">
			...
			<div class="logoutForm" id="memberinfo-logoutForm">
				<ul class="nav navbar navbar-top-links navbar-right mbn">
					...
					<li class="dropdown">
						<a data-hover="dropdown" href="#" class="dropdown-toggle">
							...
							<span class="hidden-xs" id="member-name"></span>&nbsp;<span class="caret"></span>
						</a>
						<ul class="dropdown-menu" role="menu">
							...
							<li class="divider"></li>
							<li><a href="/user/join/logout.do"><i class="fa fa-key"></i>로그아웃</a></li>
						</ul>
					</li>
				</ul>
			</div>
			<div class="loginForm nav navbar navbar-top-links navbar-right"
				style="padding: 10px;" id="memberinfo-loginForm">
				<ul>
					<li class="dropdown">
					...
					<li>&nbsp;</li>	
			        <li class="dropdown">
			        	<button type="button" class="btn btn-warning btn-sm" id="btnLogin">로그인</button>
